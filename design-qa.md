...
- 本轮删除全部快照覆盖层、几何验证管线与 `idle/entering/editing/exiting` 阶段机（净删约 900 行）。唯一状态源为草稿是否存在：点击原文在同一事务内建立草稿，真实卡片以 `.spring(duration: 0.45, bounce: 0.12)` 从静息高度一次展开到最终纸面；完成提交以 `.smooth(duration: 0.32)` 收回。原文编辑器全程同一身份，无交接、无双重曝光窗口。
- 键盘在下一个 runloop 请求焦点，与展开并行升起。转场全程可交互、可打断：展开途中点对勾会带当前速度平滑反向。
- 对用户实录（硬件键盘模式，无软件键盘）逐帧测量卡片底边后确认第二类根因：系统 tab bar 的安全区插入量异步提交——隐藏后约 700ms 才释放、恢复时立即插回，且两次均以内容快照交叉淡化落地。跟随该安全区的布局会先停在中间高度、约 500ms 后突跳 47pt 并出现整卡残影（进入）与整卡半透明闪变（退出）。修复：滚动容器 `ignoresSafeArea(.container, edges: .bottom)`，编辑高度改由「扩展视口 − 键盘裁切 − 窗口级底部安全区」计算；窗口 inset 不随 tab bar 变化，键盘（`.keyboard` 区域）仍被尊重、软件键盘升起时底边平滑跟随。tab bar 迟到的安全区提交只改变一个不可见的 `contentMargins`，闲置内容仍由它保持在悬浮标签栏之上。
- 提交触感改为复用并 `prepare()` 预热的 `UIImpactFeedbackGenerator`，且在动画事务提交后触发，规避冷启动触感引擎在转场首帧的主线程停顿（实录中退出点击处曾有约 100ms 冻结帧）。
...
| 完整 UI 套件（8 条主流程） | 8 passed、0 failed、0 skipped |
| 克隆设备实录逐帧复验（safe-area 解耦后） | 进入 ~25 个连续中间帧单段直达终态、停稳后无后跳；退出 ~30 帧连续收回、首帧无整卡闪变；二次进入/退出同样干净 |
...
## Voice conversation production revision (current)

语音对话页从纯 UI 原型改为真实管线（详见 README「对话翻译」条目）。视觉语言保持不变：气泡几何、语言圆钮、麦克风大按钮与 Liquid Glass 组合照旧。新增/变化点：

- 生产版从空白待机开始（不再自动聆听、不再种入示例对话），空态为居中 `mic.badge.plus` 提示「轻点下方按钮开始双语对话」。
- 进行中的一句话在列表底部的活动气泡里原位更新：volatile 原文 0.55 不透明度、粗译 0.45，定稿后原位实色替换（Apple Live Translation 范式，不用斜体避免文字跳动）；volatile 期间自动滚动不带动画防抖，提交后沿用原有 0.3s 动画滚动。Reduce Motion 下定稿淡化缩短至 0.12s、波形静止。
- WaveBars 由实测麦克风电平驱动（vDSP RMS，5 槽平滑环）；无电平输入时保持原关键帧循环（兼容其他调用点）。
- 状态区新增文案：轻点开始对话 / 正在准备语音识别… / 正在下载语言模型 N% / 正在翻译… / 正在朗读… / 权限错误附「前往设置」。
- 气泡加朗读按钮（`speaker.wave.2` 小圆钮，主气泡侧边、贴气泡底部对齐）；朗读会先暂停收音（半双工）。
- final 翻译失败时活动气泡内出现「翻译失败 · 轻点重试」红色文案按钮，原文与最后一次粗译保留。
- 双语自动检测（默认模式）：中间麦克风自动在语言对内识别谁在说哪种语言，气泡侧别与语言圆钮高亮跟随检测结果；点圆钮手动锁定一侧、再点回到自动。状态文案区分「正在聆听 · English / 中文」（自动）与单语言（手动）。
- 页面头部右上角新增朗读模式直达菜单（speaker.slash / speaker.wave.2 / headphones 图标随当前模式变化），与设置页「语音对话」分区同一份持久化状态。
- 底部 dock 为悬浮层（safeAreaInset）：毛玻璃 + 纸色 55%，顶部 32% 渐隐遮罩收边——滚动内容从按钮后方模糊淡出，无硬边界截断。
- 断句不再闪相位：定稿是识别流上的切分点，麦克风按钮与状态全程稳定「正在聆听」；译文异步填充把最后一条气泡撑高时列表跟滚到底（含失败重试按钮出现时）。

验收：完整 UI 套件九条流程 + 单元测试见 README「自动化验收」；SpeechAnalyzer/系统翻译真机项见下方边界。

## Acceptable prototype boundaries

- 文字翻译已接谷歌免费接口；语音对话已接真实识别+翻译管线；菜单 OCR 仍使用本地演示数据。
- 模拟器上 SpeechTranscriber 与 Translation 框架不可用（苹果官方约束）：模拟器实测走 SFSpeechRecognizer + 谷歌回退；SpeechAnalyzer 路径、系统离线翻译、语言模型下载、耳机检测与端点阈值手感属于真机验收项，待设备在线后完成。
